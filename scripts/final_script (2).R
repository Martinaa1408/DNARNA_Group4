...
# Normalize probe type to handle both character and numeric values
type_char <- if (actual_type %in% c("I", 1)) {
  "I"
} else if (actual_type %in% c("II", 2)) {
  "II"
} else {
  "Not found"
}

# Check probe details based on normalized type
if (type_char == "I") {
  
  # Look for the probe in both AddressA_ID and AddressB_ID
  probe_info <- Illumina450Manifest_clean[
    Illumina450Manifest_clean$Infinium_Design_Type == "I" &
      (Illumina450Manifest_clean$AddressA_ID == address |
         Illumina450Manifest_clean$AddressB_ID == address), ]
  
  if (nrow(probe_info) == 0) {
    stop("Address not found among Type I probes.")
  }
  
  # Extract color
  color <- as.character(probe_info$Color_Channel[1])
  
  # Determine methylated and unmethylated addresses
  if (probe_info$AddressA_ID[1] == address) {
    meth_address <- address
    unmeth_address <- as.character(probe_info$AddressB_ID[1])
  } else {
    meth_address <- as.character(probe_info$AddressA_ID[1])
    unmeth_address <- address
  }
  
  # Extract intensities from the correct channel
  if (color == "Red") {
    meth_values <- Red[meth_address, ]
    unmeth_values <- Red[unmeth_address, ]
  } else if (color == "Green") {
    meth_values <- Green[meth_address, ]
    unmeth_values <- Green[unmeth_address, ]
  } else {
    stop("Invalid or missing color channel.")
  }
  
  # Create output table
  df_address <- data.frame(
    Sample = colnames(Green),
    Red_fluor = unlist(Red[address, ], use.names = FALSE),
    Green_fluor = unlist(Green[address, ], use.names = FALSE),
    Type = type_char,
    Color = color
  )
  
} else if (type_char == "II") {
  
  # Type II probes use one address across both channels
  meth_values <- Green[address, ]
  unmeth_values <- Red[address, ]
  
  df_address <- data.frame(
    Sample = colnames(Green),
    Red_fluor = unlist(unmeth_values, use.names = FALSE),
    Green_fluor = unlist(meth_values, use.names = FALSE),
    Type = type_char,
    Color = "Red+Green"
  )
  
} else {
  stop(paste("Probe type", type_char, "not recognized or address not found in the manifest."))
}

# Print final table including Color column
knitr::kable(
  df_address,
  caption = paste("Methylation signals for address", address),
  col.names = c("Sample", "Red_fluor", "Green_fluor", "Type", "Color"),
  align = "lrrcc"
)
...
# Fast method using rowttests from genefilter package
